...
switch ($requestMethod) {
    case 'GET':
        if (isset($_GET['batiment']) && isset($_GET['cote']) && isset($_GET['secteur'])) { // Vérification des paramètres
            ...
            if (empty($row_exact)) { // Si on a pas de résultats pour la correspondance exacte
                if (is_numeric($cote[0])) { // Si c'est numérique on fait un range
                    ...
                    if (empty($row_range)) { // Si le range ne donne pas de résultats, on renvoie les informations sur le bâtiment + les images
